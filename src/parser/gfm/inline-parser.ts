...
import {
  parseStrikethroughDelimiter,
  StrikethroughDelimiter,
  canStrikethroughDelimitersMatch,
  createStrikethroughNode,
} from '../inlines/strikethrough.js';
...
interface StrikethroughDelimiterEntry {
  run: StrikethroughDelimiter;
  node: TextNode;
  active: boolean;
  nodeIndex: number;
}
...
export class GFMInlineParser {
  ...
  private enableStrikethrough: boolean;
  private enableFootnotes: boolean;
  private enableTagfilter: boolean;
  ...
  private processEmphasisAndStrikethrough(nodes: InlineNode[], stackBottom: Delimiter | null): void {
  ...
  private processStrikethrough(nodes: InlineNode[]): void {
    const delimiters: StrikethroughDelimiterEntry[] = [];
  ...
  private flattenNestedEmphasis(nodes: InlineNode[]): void {
    for (const node of nodes) {
      if ('children' in node && Array.isArray(node.children)) {
        this.flattenNestedEmphasis(node.children as InlineNode[]);
        if (node.type === 'strong') {
          const flattened: InlineNode[] = [];
          for (const child of node.children as InlineNode[]) {
            if (child.type === node.type) {
              flattened.push(...(child.children as InlineNode[]));
            } else {
